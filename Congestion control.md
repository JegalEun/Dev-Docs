...
`Tahoe` 이후에 나온 버전으로 **손실의 종류**에 따라 `cwnd`를 수정하는 방식이 다르다. 우선, **3개의 중복 ACK에 의한 손실이 발생**하면 `cwnd` 크기를 `1MSS`로 줄이는게 아니라 현재 **cwnd 크기의 절반**으로 줄인다음 `1MSS`씩 선형으로 증가를 시킨다. 하지만 만약 **타임아웃에 의한 손실**이라면 `Tahoe`와 마찬가지로 **`1MSS`로 초기화** 되고 `Slow Start`를 진행한다.
